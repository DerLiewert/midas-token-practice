(() => {
    ...
    function initSliders() {
        if (document.querySelector(".slider")) new core(".slider", {
        ...
        if (document.querySelector(".others-proj__slider")) new core(".others-proj__slider", {
            modules: [ Navigation, Scrollbar, Lazy ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            ...
            scrollbar: {
                el: ".others-proj__scrollbar",
                draggable: true,
                dragClass: "slider-scrollbar-drag"
            },
            navigation: {
                prevEl: ".others-proj__button_prev",
                nextEl: ".others-proj__button_next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1,
    ...
    window.addEventListener("load", (function(e) {
        initSliders();
        if (document.querySelector(".swiper_scroll")) new core(".swiper_scroll", {
            modules: [ freeMode ],
            slidesPerView: "auto",
            freeMode: true,
            watchOverflow: true
...
***************************************************************************** */
    ...
    function filters() {
        ...
        if (filters.length > 0) {
            filters.forEach((filterBlock => {
                const isDeleteItems = void 0 !== filterBlock.dataset.filterDelItems;
                const categories = filterBlock.querySelectorAll("[data-filter-category]");
                const itemsWrapper = filterBlock.querySelector("[data-filter-items]");
                let items = itemsWrapper.querySelectorAll("[data-filter-item]");
                let itemsCopy = items;
                let activeCategory = filterBlock.querySelector("[data-filter-category]._active-category");
                if (!activeCategory) {
                    activeCategory = categories[0];
                    activeCategory.classList.add("_active-category");
                }
                showItemsByCategory(isDeleteItems, activeCategory, items, itemsCopy);
                categories.forEach((category => {
                    category.addEventListener("click", (() => {
                        if (category.classList.contains("_active-category")) return;
                        if (isDeleteItems) items = filterBlock.querySelectorAll("[data-filter-item]");
                        category.classList.add("_active-category");
                        activeCategory.classList.remove("_active-category");
                        activeCategory = category;
                        showItemsByCategory(isDeleteItems, activeCategory, items, itemsCopy);
                        showMorePortfolio(filterBlock);
                        if (itemsWrapper.hasAttribute("data-watch")) {
                            itemsWrapper.classList.remove("_watcher-view");
                            setTimeout((() => itemsWrapper.classList.add("_watcher-view")), 0);
                        }
                    }));
                }));
            }));
            function showItemsByCategory(isDeleteItems, activeCategory, items, itemsCopy) {
                if (isDeleteItems) {
                    const node = items[0].closest("[data-filter-items]");
                    if ("all" !== activeCategory.dataset.filterCategory) {
                        while (node.firstChild) node.removeChild(node.firstChild);
                        itemsCopy.forEach((item => {
                            let isShow = false;
                            item.dataset.filterItem.split(" ").forEach((category => {
                                if (category === activeCategory.dataset.filterCategory) isShow = true;
                            }));
                            if (isShow) node.append(item);
                        }));
                    } else itemsCopy.forEach((item => node.append(item)));
                } else if ("all" !== activeCategory.dataset.filterCategory) items.forEach((item => {
                    let isShow = false;
                    item.dataset.filterItem.split(" ").forEach((category => {
                        if (category === activeCategory.dataset.filterCategory) isShow = true;
                    }));
                    if (isShow) {
                        ...
                        setTimeout((() => item.classList.add("_show-anim")), 0);
                    } else {
                        item.style.display = "none";
                        item.classList.remove("_show-anim");
                    }
                })); else items.forEach((item => {
                    item.style.display = "block";
                    item.classList.add("_show-anim");
                }));
            }
        }
    }
    filters();
    const portfolios = document.querySelectorAll(".portfolio__body");
    if (portfolios.length > 0) portfolios.forEach((portfolio => {
        showPortfolioItemsCount(portfolio);
        showMorePortfolio(portfolio);
    }));
    function showPortfolioItemsCount(portfolio) {
        const filterCategories = portfolio.querySelectorAll(".portfolio__category");
        const filterItems = portfolio.querySelectorAll(".portfolio__project");
        for (let i = 0; i < filterCategories.length; i++) {
            const category = filterCategories[i];
            if ("all" === category.dataset.filterCategory) {
                category.querySelector(".portfolio__category-count").append(filterItems.length);
                continue;
            }
            let counter = 0;
            filterItems.forEach((item => {
                if (item.dataset.filterItem.includes(category.dataset.filterCategory)) counter++;
            }));
            category.querySelector(".portfolio__category-count").append(counter);
        }
    }
    const categoriesBlocks = document.querySelectorAll(".portfolio__categories");
    if (categoriesBlocks.length) categoriesBlocks.forEach((categoriesBlock => {
        const hoverLine = categoriesBlock.querySelector(".portfolio__categories-hover-line");
        if (hoverLine) {
            hoverLine.style.width = categoriesBlock.querySelector(".portfolio__category._active-category .portfolio__category-name")?.offsetWidth + "px";
            categoriesBlock.addEventListener("mouseover", (e => {
                const el = e.target.closest(".portfolio__category");
                if (el) {
                    hoverLine.style.left = el.offsetLeft + "px";
                    hoverLine.style.width = el.querySelector(".portfolio__category-name").offsetWidth + "px";
                }
            }));
            categoriesBlock.addEventListener("mouseleave", (() => {
                const activeCategory = categoriesBlock.querySelector(".portfolio__category._active-category");
                if (activeCategory) {
                    hoverLine.style.left = activeCategory.offsetLeft + "px";
                    hoverLine.style.width = activeCategory.querySelector(".portfolio__category-name").offsetWidth + "px";
                }
            }));
            window.addEventListener("resize", (() => hoverLine.style.width = categoriesBlock.querySelector(".portfolio__category._active-category .portfolio__category-name")?.offsetWidth + "px"));
        }
    }));
    function showMorePortfolio(portfolio) {
        let portfolioItems = portfolio.querySelectorAll(".portfolio__project");
        const showMoreButton = portfolio.querySelector(".portfolio__button");
        if (portfolioItems.length > 0) {
            if (!portfolio?.hasAttribute("data-filter-del-items")) {
                const temp = [];
                portfolioItems.forEach((item => {
                    if (item.classList.contains("_show-anim")) temp.push(item);
                }));
                portfolioItems = temp;
            }
            const activeCategory = portfolio.querySelector("._active-category")?.getAttribute("data-filter-category");
            let showItemCount = 10;
            showMoreItems(portfolioItems, showItemCount);
            if (portfolioItems.length <= showItemCount) showMoreButton.closest(".portfolio__button-wrapper").style.display = "none"; else showMoreButton.closest(".portfolio__button-wrapper").style.removeProperty("display");
            showMoreButton.addEventListener("click", (() => {
                showItemCount += showItemCount;
                showMoreItems(portfolioItems, showItemCount);
                if (portfolioItems.length <= showItemCount) showMoreButton.closest(".portfolio__button-wrapper").style.display = "none";
            }));
            function showMoreItems(items, showItemCount) {
                let showCounter = 0;
                if ("all" === activeCategory) items.forEach(((item, index) => {
                    ...
                    if (item.getAttribute("data-filter-item") === activeCategory && showCounter < showItemCount) {
                        item.style.display = "block";
                        item.classList.add("_show-anim");
                        showCounter++;
                    } else {
        ...
        } else showMoreButton.closest(".portfolio__button-wrapper").style.display = "none";
    }
    function showArrowGoToStart() {
        const arrowGoStart = document.querySelector(".arrow-go-start");
        document.addEventListener("scroll", (function(e) {
            const scrollTop = window.scrollY;
            if (scrollTop >= window.screen.height / 4) !arrowGoStart.classList.contains("_go-start-show") ? arrowGoStart.classList.add("_go-start-show") : null; else arrowGoStart.classList.contains("_go-start-show") ? arrowGoStart.classList.remove("_go-start-show") : null;
        }));
    }
    showArrowGoToStart();
    document.querySelectorAll(".lg-prev.lg-icon").forEach((btn => {
        btn.innerHTML = `<svg width="21" height="8" viewBox="0 0 21 8" xmlns="http://www.w3.org/2000/svg">\n            <path d="M0.314415 4.35355C0.119154 4.15829 0.119154 3.84171 0.314415 3.64645L3.4964 0.464466C3.69166 0.269204 4.00824 0.269204 4.2035 0.464466C4.39877 0.659728 4.39877 0.976311 4.2035 1.17157L1.37508 4L4.2035 6.82843C4.39877 7.02369 4.39877 7.34027 4.2035 7.53553C4.00824 7.7308 3.69166 7.7308 3.4964 7.53553L0.314415 4.35355ZM20.668 4.5L0.667969 4.5V3.5L20.668 3.5V4.5Z"/>\n        </svg>`;
    }));
    document.querySelectorAll(".lg-next.lg-icon").forEach((btn => {
        btn.innerHTML = `<svg width="21" height="8" viewBox="0 0 21 8" xmlns="http://www.w3.org/2000/svg">\n            <path d="M0.314415 4.35355C0.119154 4.15829 0.119154 3.84171 0.314415 3.64645L3.4964 0.464466C3.69166 0.269204 4.00824 0.269204 4.2035 0.464466C4.39877 0.659728 4.39877 0.976311 4.2035 1.17157L1.37508 4L4.2035 6.82843C4.39877 7.02369 4.39877 7.34027 4.2035 7.53553C4.00824 7.7308 3.69166 7.7308 3.4964 7.53553L0.314415 4.35355ZM20.668 4.5L0.667969 4.5V3.5L20.668 3.5V4.5Z"/>\n        </svg> `;
    }));
    const priceBlocks = document.querySelectorAll(".price__items-wrapper");
    priceBlocks.forEach((priceBlock => {
        const priceItems = priceBlock.querySelectorAll(".price__item.price-item");
        priceItems.forEach((item => {
            const priseList = item.querySelector(".price-item__list");
            const priceButton = item.querySelector(".price-item__show-hide");
            _slideUp(priseList);
            priceButton.addEventListener("click", (() => itemsCloseOpen(priceButton, priseList)));
        }));
        const totalPriceButton = priceBlock.querySelector(".price__button");
        if (totalPriceButton) {
            totalPriceButton.innerHTML = "развернуть";
            totalPriceButton.classList.remove("_open");
            totalPriceButton.addEventListener("click", (() => itemsCloseOpen(totalPriceButton, priceItems)));
        }
        if (priceItems.length && totalPriceButton) {
            let isTablet = false;
            window.addEventListener("resize", (() => {
                if (document.body.offsetWidth < 992 && !isTablet) {
                    priceItems.forEach((item => {
                        const priseList = item.querySelector(".price-item__list");
                        const priceButton = item.querySelector(".price-item__show-hide");
                        itemsCloseOpen(priceButton, priseList, true);
                    }));
                    isTablet = true;
                } else if (document.body.offsetWidth >= 992 && isTablet) {
                    itemsCloseOpen(totalPriceButton, priceItems, true);
                    isTablet = false;
                }
            }));
        }
        function itemsCloseOpen(priceButton, priceItemsOrItemList, isOpen = null) {
            isOpen = isOpen ? isOpen : priceButton.classList.contains("_open");
            priceButton.disabled = true;
            if (isOpen) {
                priceButton.classList.remove("_open");
                priceButton.innerHTML = "развернуть";
            } else {
                priceButton.classList.add("_open");
                priceButton.innerHTML = "Свернуть";
            }
            if (priceItemsOrItemList.length) priceItemsOrItemList.forEach((priceItem => {
                const priseList = priceItem.querySelector(".price-item__list");
                isOpen ? _slideUp(priseList) : _slideDown(priseList);
            })); else isOpen ? _slideUp(priceItemsOrItemList) : _slideDown(priceItemsOrItemList);
            window.setTimeout((() => {
                priceButton.disabled = false;
            }), 500);
        }
    }));
    function script2_stickyBlock() {
        const stickyParents = document.querySelectorAll("[data-sticky]");
        if (stickyParents.length) stickyParents.forEach((stickyParent => {
            let stickyConfig = {};
            let stickyBlockItem = null;
            let headerHeight = null;
            let offsetTop = null;
            let startPoint = null;
            if ("static" === window.getComputedStyle(stickyParent).position) stickyParent.style.position = "relative";
            stickyInit();
            stickyBlockActions();
            ...
            function stickyInit() {
                stickyConfig = {
                    ...
                    header: stickyParent.hasAttribute("data-sticky-header") ? document.querySelector("header.header").offsetHeight + 15 : 0
                };
                stickyBlockItem = stickyParent.querySelector("[data-sticky-item]");
                headerHeight = stickyConfig.header;
                offsetTop = headerHeight + stickyConfig.top;
                startPoint = stickyParent.getBoundingClientRect().top + scrollY - offsetTop;
            }
            function stickyBlockActions(e) {
                ...
                if (!stickyConfig.media || stickyConfig.media <= window.innerWidth) if (offsetTop + stickyConfig.bottom + stickyBlockItem.offsetHeight < window.innerHeight) if (scrollY >= startPoint && scrollY <= endPoint) {
                    ...
                    stickyItemValues.left = `${stickyParent.getBoundingClientRect().left}px`;
                ...
                stickyBlockStyle(stickyBlockItem, stickyItemValues);
            }
        }));
        function stickyBlockStyle(stickyBlockItem, stickyItemValues) {
    ...
    window.addEventListener("load", (function() {
        setTimeout((function() {
            script2_stickyBlock();
            if (document.querySelector(".why__title")) {
                document.querySelector(".why__title").classList.add("_show-anim");
                setTimeout((() => {
                    document.querySelector(".why__title").classList.remove("_show-anim");
                }), 0);
            }
        }), 600);
    }));
    ...
    menuInit();
    window.addEventListener("resize", (() => {
        if (window.innerWidth > 767.98 && document.documentElement.classList.contains("menu-open")) {
            document.documentElement.classList.remove("menu-open");
            bodyUnlock();
        }
    }));
